...
def load_csvs(os_path, duplicate_file_path):
	...
	for folder in csv_paths:
		...
		val_idf = idf.join(idf.groupBy(idf.columns).agg((
		f.count("*")>1).cast("int").alias(
		"Duplicate_indicator")), on=idf.columns, how="inner")
		...
		val_cldf = cldf.join(cldf.groupBy(cldf.columns).agg((
		f.count("*")>1).cast("int").alias(
		"Duplicate_indicator")), on=cldf.columns, how="inner")
		df2 = val_cldf.where("Duplicate_indicator > 0")
		df2.toPandas().to_csv(duplicate_file_path+folder+'/'+'clicks_duplicates'+'_'+folder+'.csv')
		
		val_codf = codf.join(codf.groupBy(codf.columns).agg((
		f.count("*")>1).cast("int").alias(
		"Duplicate_indicator")), on=codf.columns, how="inner")
...
def main():
	...
	try:
		...
		for ints in range(len(file_data)):
			collection_banners.save(file_data[ints])
	
	except pymongo.errors.DuplicateKeyError:
		print ('You are trying to insert duplicate data on (campaign_id, time_quarter)')
